...
# ---------------------------------------------------------------------------
#  Illustrations : format de référence et redimensionnement automatique
# ---------------------------------------------------------------------------
# Toutes les illustrations du jeu partagent le même format (255 x 257 px,
# PNG). Pour compléter le jeu, il suffit désormais de déposer une image à ce
# format dans le dossier "images" sous le nom attendu (imageXX.png) : aucune
# autre modification n'est nécessaire.
#
# Par sécurité, si Pillow (PIL) est installé, toute image qui ne serait pas
# exactement à ce format est automatiquement redimensionnée à l'affichage
# (sans déformer le programme ni planter le jeu). Sans Pillow, le jeu
# continue de fonctionner comme avant (chargement direct via tk.PhotoImage) :
# dans ce cas, les images doivent être exactement au format 255 x 257 pour
# s'afficher correctement et de façon homogène.
IMAGE_SIZE = (255, 257)  # (largeur, hauteur) — format des illustrations existantes
...
def _load_display_image(path, size=IMAGE_SIZE):
    """Charge une image de passage et la renvoie prête à afficher, toujours
    au format `size`. Utilise Pillow si disponible (redimensionnement
    propre, avec anti-crénelage) ; sinon retombe sur tk.PhotoImage brut
    (l'image doit alors déjà être au bon format)."""
    if _PIL_AVAILABLE:
...
class AdventureGame:
    ...
    def display_story(self, part_id):
        ...
        # -- mise en forme des lignes spéciales de fin de chemin -----------
        # Le séparateur "✦ ✦ ✦" est centré, et la signature du Veilleur du
        # Seuil ("— ...") est alignée à droite, comme une citation que l'on
        # attribue en fin de texte. Le reste du texte garde l'alignement à
        # gauche habituel.
        self.text_label.tag_configure("centered", justify="center")
        self.text_label.tag_configure("right_aligned", justify="right")

        line_count = int(self.text_label.index("end-1c").split(".")[0])
        for line_no in range(1, line_count + 1):
            line_content = self.text_label.get(f"{line_no}.0", f"{line_no}.end")
            stripped = line_content.strip()
            if not stripped:
                continue
            if set(stripped) <= {"✦", " "}:
                self.text_label.tag_add("centered", f"{line_no}.0", f"{line_no}.end")
            elif stripped.startswith("—"):
                self.text_label.tag_add("right_aligned", f"{line_no}.0", f"{line_no}.end")
