...
story = {
    ...
    204: {
        ...
        "choices": {
            ...
            "3": {"text": "Brûler le grimoire pour éviter qu'il ne tombe entre de mauvaises mains", "next_part": 215}
        }
    },
    ...
    237: {
        ...
        "choices": {
            "1": {"text": "Affronter l'esprit pour obtenir l'artefact", "next_part": 296},
            "2": {"text": "Négocier avec l'esprit pour obtenir l'artefact", "next_part": 297},
            "3": {"text": "Fuir la caverne et chercher ailleurs", "next_part": 298}
        }
    },
    ...
    238: {
        ...
        "choices": {
            "1": {"text": "Résoudre l'énigme pour entrer dans la caverne", "next_part": 299},
            "2": {"text": "Chercher des indices supplémentaires", "next_part": 300},
            "3": {"text": "Abandonner l'exploration de la caverne", "next_part": 301}
        }
    },
    ...
    239: {
        ...
        "choices": {
            "1": {"text": "Explorer la forêt enchantée", "next_part": 302},
            "2": {"text": "Chercher des indices sur les créatures magiques", "next_part": 303},
            "3": {"text": "Revenir à la vallée pour explorer d'autres lieux", "next_part": 304}
        }
    },
...
def get_story_part(part_id):
    """Retourne le texte de la partie de l'histoire correspondant à part_id et les choix disponibles."""
    part = story.get(part_id, None)
    if part is None:
        return "Partie de l'histoire non trouvée.", {}
    return part["text"], part["choices"]

def get_next_part(part_id, choice):
    """Retourne l'ID de la prochaine partie de l'histoire en fonction du choix de l'utilisateur."""
    part = story.get(part_id, None)
    if part is None:
        return None
    return part["choices"].get(choice, {}).get("next_part", None)

def get_choice_text(part_id, choice):
    """Retourne le texte associé à un choix spécifique pour une partie donnée."""
    part = story.get(part_id, None)
    if part is None:
        return "Choix non trouvé."
    return part["choices"].get(choice, {}).get("text", "Choix non trouvé.")
